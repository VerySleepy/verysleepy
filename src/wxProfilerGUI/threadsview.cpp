...
void ThreadsView::OnSelected(wxListEvent &event)
{
	startSelectionTimer();
}

void ThreadsView::OnDeSelected(wxListEvent &event)
{
	startSelectionTimer();
}
...
void ThreadsView::OnTimer(wxTimerEvent &event)
{
...
void ThreadsView::fillList()
{
	...
	for (int i = 0; i < threads.size(); ++i) {
		...
		char str[32];
		snprintf(str, ARRAYSIZE(str), "%d", threads[i].tid);
		SetItem(i, COL_TID, str);
